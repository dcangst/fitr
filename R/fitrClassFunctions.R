...
print.fitr <- function(fitr) {
  ...
  failPercent <- signif(
    dim(fitr$bestfits[fitr$bestfits$comment != "ok", c(1, 11)])[1] / length(datasum$ID) * 100,
    digits = 3)
  ...
  cat("$data", "\n")
  cat("\t", length(datasum$ID), " growthcurves with ",
    min(datasum$minT, na.rm = TRUE), " to ", max(datasum$maxT, na.rm = TRUE), " timepoints", sep = "")
  cat("\n")
...
#' @param description data.frame containing description of your curves, must contain
#'  ID column with unique ID for each curve.
...
write.fitr <- function(fitr, basefile = paste0(format(Sys.time(), "%Y%m%d"), "fitR"), description = FALSE, sep = ";") {
  ...
  sumtext1 <- paste(length(datasum$ID), " growthcurves with ",
    min(datasum$minT, na.rm = TRUE), " to ", max(datasum$maxT, na.rm = TRUE), " timepoints", sep = "")
  sumtext2 <- paste("No valid fit for ", failN, " out of ", length(datasum$ID),
    " (", failPercent, "%) curves.", sep = "")

  write.table(paste0("growthcurves fitted with fitR v.", packageVersion("fitr")), paste0(basefile, "_summary.csv"),
    row.names = FALSE, col.names = FALSE, sep = sep)
  write.table(sumtext1, paste0(basefile, "_summary.csv"),
    row.names = FALSE, col.names = FALSE, append = TRUE, sep = sep)
  write.table(sumtext2, paste0(basefile, "_summary.csv"),
    row.names = FALSE, col.names = FALSE, append = TRUE, sep = sep)
  write.table("$bestfits", paste0(basefile, "_summary.csv"),
    row.names = FALSE, col.names = FALSE, append = TRUE, sep = sep)
  if (description == FALSE) {
    write.table(fitr$bestfits, paste0(basefile, "_summary.csv"), append = TRUE, row.names = FALSE, sep = sep)
  } else {
    write.table(merge(description, fitr$bestfits, by = c("ID")), paste0(basefile, "_summary.csv"),
